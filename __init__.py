...
class ElectoralSystem(ABC):
    ...
    def _make_result(
        ...
        outcome = candidates.positions[winner_idx]
        return ElectionResult(
            outcome_position=outcome.copy(),
    ...
    def _make_pr_result(
        ...
        """Helper: build a PR ElectionResult with weighted centroid outcome."""
        outcome = np.zeros(candidates.n_dims)
        for idx, share in seat_shares.items():
            outcome += share * candidates.positions[idx]
