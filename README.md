...
The main notebook reproduces all figures and tables from the paper:

```bash
jupyter nbconvert --to notebook --execute \
  notebooks/01_electoral_systems_comparison.ipynb
```

Or open it interactively in JupyterLab. All results use a fixed seed (`SEED=42`) for reproducibility.
...
All 25 tests validate known theoretical properties:
- IRV and Condorcet elect the candidate nearest the median in 1D unimodal electorates
- Pairwise matrix rows sum to 1
- PR seat shares sum to 1
- All systems handle single-candidate elections correctly
- Geometric median recovers known properties in symmetric distributions
...
**Geometric median as primary metric.** Minimises sum of Euclidean distances; more robust than the mean in polarised electorates; unique in $\mathbb{R}^N$; connects to the median voter theorem in 1D.

**Unified result interface.** Every system returns an `ElectionResult` dataclass with a common set of fields, making cross-system comparison straightforward without special-casing.

**YAML-defined scenarios.** Electorate distributions and candidate configurations are fully specified in plain-text config files, so new scenarios require no Python code.
